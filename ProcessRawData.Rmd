...
base=annualGlobal %>%
  filter(year>=syear) %>%
  filter(year<=eyear)
baseline=(sum(base$anomaly))
basespread=(max(base$anomaly)-min(base$anomaly))

stats=glue("The total series spread id {seriesSpread} and the baseline anomaly for the years {syear} through {eyear} inclusive is {baseline} with a spread of {basespread}")
print(stats)
