...
for(i in seq(1,n/2)){
  ...
  raw<- (with(hstations,readMet(names[i],paste0('metdata/',fname[i])  )))
  loc<-raw[raw %like% "Location:"]
  locations<-c(locations,loc)
  fdata<-which(raw %like% "degC")
  rmetdata<-read_delim(raw[fdata+1:length(raw)],delim=" ",col_names = c("year","month","tmax","tmin","af","rain","sun"),trim_ws=TRUE,na="---")
  rmetdata$station=hstations$station[i]
